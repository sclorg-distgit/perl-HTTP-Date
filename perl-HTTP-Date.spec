%{?scl:%scl_package perl-HTTP-Date}

Name:           %{?scl_prefix}perl-HTTP-Date
Version:        6.05
Release:        3%{?dist}
...
BuildRequires:  %{?scl_prefix}perl(Time::Local) >= 1.28
# Time::Zone not used
# Tests:
...
Requires:       %{?scl_prefix}perl(Time::Local) >= 1.28
# Strongly recommended:
Requires:       %{?scl_prefix}perl(Time::Zone)
Conflicts:      %{?scl_prefix}perl-libwww-perl < 6

# Remove under-specified version
%global __requires_exclude %{?__requires_exclude:%{__requires_exclude}|}^%{?scl_prefix}perl\\(Time::Local\\)$
...
%changelog
* Thu Feb 13 2020 Petr Pisar <ppisar@redhat.com> - 6.05-3
- Import to SCL
